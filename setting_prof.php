...
if (isset($_POST['sub']) && sizeof($_POST) > 0 && isset($_SESSION['usnm'])) {
    extract($_POST);
    // echo
    $sl_qr = "SELECT * FROM `users` WHERE `pass`='$crt_pass' && `user_id`=$_SESSION[user_id]";
    // exit;
    $sl_res = mysqli_query($conn, $sl_qr);
    $data = mysqli_num_rows($sl_res);
    if ($data == 1) {
        if ($new_pass == $rpt_pass) {
            $qr = "UPDATE `users` SET `pass`='$new_pass' where `user_id`=" . $_SESSION['user_id'];
